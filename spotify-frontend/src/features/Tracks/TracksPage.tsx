      ...
      {tracks.length > 0 ? (
        <>
          <div style={{display: 'flex', justifyContent: 'space-between',}}>
            ...
            {user && (
              <Button variant={'outlined'} sx={{margin: '20px 70px', height: '100%'}} component={NavLink} to={'/new-track'}>
                Add Track
              </Button>
            )}
          </div>
        ...
        <div style={{display: 'flex', justifyContent: 'center', flexDirection: 'column', alignItems: 'center', marginTop: '20px'}}>
          ...
          {user && (
            <Button variant={'contained'} color="primary" component={NavLink} to={'/new-track'}>
              Создать трек
            </Button>
          )}
        </div>
